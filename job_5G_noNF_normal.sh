...
export BASE=5G_noNF_normal

#SBATCH -J 5G_noNF_normal # A single job name for the array
#SBATCH -n 8
#SBATCH -t 8:00:00 # timeout
#SBATCH -o 5G_noNF_normal/5G-%A%a.out # Standard output
#SBATCH -e 5G_noNF_normal/5G-%A%a.err # Standard error

mkdir -p $BASE

LEMMAS=('rand_autn_src' 'sqn_src' 'sqn_ue_increase' 'sqn_ue_unique' 'rand_sources_1' 'rand_sources_2' 'autn_sources_1' 'autn_sources_2' 'relay_SMC_sources' 'UE_sec_capabilities_sources' 'SMComplete_sources' '5G_HE_AV_sources' 'Attach_SUPI_sources' 'UE_sec_capabilities_attach_sources' 'RES_star_sources' 'secrecy_Ki' 'secrecy_AUSF' 'secrecy_SEAF' 'secrecy_KAMF' 'secrecy_KgNB')
...
--prove=${LEMMAS[$ID]} \
--output=$BASE/output_${LEMMAS[$ID]} +RTS -N8 -RTS 2>/dev/null
